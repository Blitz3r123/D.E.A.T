let quickCreateGenSettingVals = readData( path.join(__dirname, '../data/CreateGeneralSettings.json') );
...
function updateSetting(e){
    ...
    let allSettings = JSON.parse(fs.readFileSync( path.join( __dirname, '../data/CreateGeneralSettings.json' ) ));
    ...
    fs.writeFile(path.join( __dirname, '../data/CreateGeneralSettings.json' ), JSON.stringify(allSettings), err => {
        if(err){
            console.log(err);
        }else{
            console.log('Updated CreateGeneralSettings.json!');
        }
    });
}